...
$APPLICATION->SetTitle('Канбан: задачи креативного отдела (КО)');
...
$activeStatuses = [2, 3];
$nowTs = time();
$todayStartTs = strtotime('today', $nowTs);
$tomorrowStartTs = strtotime('tomorrow', $nowTs);
$weekEndTs = strtotime('sunday this week 23:59:59', $nowTs);
$nextWeekStartTs = strtotime('monday next week 00:00:00', $nowTs);
$nextWeekEndTs = strtotime('sunday next week 23:59:59', $nowTs);

$eventType = 'MARKETING_KANBAN_KO_VISIT';
...
if ($currentUserId > 0 && !in_array($currentUserId, $skipLoggingUserIds, true)) {
    CEventLog::Add([
        ...
        'MODULE_ID' => 'main',
        'ITEM_ID' => 'forms/marketing/view_tasks_Kanban_KO.php',
...
$rows = array_values(array_filter($rows, static function (array $task): bool {
    return strpos((string)($task['TITLE'] ?? ''), 'КО') !== false;
}));
...
$columns = [
    'today' => ['title' => 'На сегодня', 'tasks' => []],
    'week' => ['title' => 'На этой неделе', 'tasks' => []],
    'next_week' => ['title' => 'На следующей неделе', 'tasks' => []],
    'no_deadline' => ['title' => 'Без срока', 'tasks' => []],
    'later' => ['title' => 'Больше двух недель', 'tasks' => []],
];

foreach ($rows as $task) {
    $task['IMAGE_PREVIEWS'] = $getTaskImagePreviews($task);
    $deadlineTs = $task['DEADLINE_TS'];
    if (!$deadlineTs) {
        $columns['no_deadline']['tasks'][] = $task;
    } elseif ($deadlineTs < $tomorrowStartTs) {
        $columns['today']['tasks'][] = $task;
    } elseif ($deadlineTs <= $weekEndTs) {
        $columns['week']['tasks'][] = $task;
    } elseif ($deadlineTs >= $nextWeekStartTs && $deadlineTs <= $nextWeekEndTs) {
        $columns['next_week']['tasks'][] = $task;
    } else {
        $columns['later']['tasks'][] = $task;
    }
}
